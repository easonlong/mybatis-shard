...
public class ProxyClassPathMapperScanner extends ClassPathBeanDefinitionScanner {
	...
	public Set<BeanDefinitionHolder> doScan(String... basePackages) {
		...
		if (beanDefinitions.isEmpty()) {
			...
			for (BeanDefinitionHolder holder : beanDefinitions) {
				...
				definition.getPropertyValues().add("writePatterns", writePatterns);
				definition.getPropertyValues().add("readPatterns", readPatterns);

				if (StringUtils.hasText(this.writeSqlSessionFactoryBeanName)) {
					definition.getPropertyValues().add("writeSqlSessionFactory",
					        new RuntimeBeanReference(this.writeSqlSessionFactoryBeanName));
	...
	protected boolean checkCandidate(String beanName, BeanDefinition beanDefinition) throws IllegalStateException {
		if (super.checkCandidate(beanName, beanDefinition)) {
			...
			logger.warn("Skipping WriteReadMapperProxyFactoryBean with name '" + beanName + "' and '"
	...
	public void setReadSqlSessionFactoryBeanNames(List<String> readSqlSessionFactoryBeanNames) {
